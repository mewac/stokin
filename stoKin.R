...
stockin <- function(parameters_df, inventory_data, demand_data, orderleadtime, delivery_leadtime, product_service_levels, order_leadtime_filter = NULL) {
  if (is.null(parameters_df) || nrow(parameters_df) == 0) {
    stop("Parameter data frame is empty.")
  }

  # Calculate seasonal factors and join with demand data
  seasonal_factors_df <- calculate_seasonal_factors(demand_data)
  ...
  for (i in 1:nrow(parameters_df)) {
    ...
    for (lt in 1:orderleadtime) {
      if (lt <= delivery_leadtime) {
        ...
        seasonal_demand <- demand * seasonal_factor
      ...
      if (is.null(order_leadtime_filter) || lt == order_leadtime_filter) {
        product_data <- data.frame(
          ...
          OrderQuantity = order_quantity[lt],
          Colisage = colisages_needed
        )
